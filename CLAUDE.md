...
- `diploma-writer` — пише/редагує текст у `diploma.docx` (жива українська, анти-ШІ, антиплагіат); факти бере від `thesis-assistant`/коду.
- `diploma-reviewer` — суворий рецензент-нормоконтролер: звіряє `diploma.docx` з `vimogi.pdf` (ДСТУ 3008:2015) і кодом, ловить помилки, ознаки ШІ та плагіат. Не редагує.

## Дипломна робота (`diploma.docx`)
Звіт **уже готовий (80+ с.) — це основа, не чернетка**. Працювати ТІЛЬКИ точково: виправляти,
змінювати, доповнювати, виправляти помилки. **Не переписувати розділи й тим паче всю роботу.**
Звіт спирається **на реальний код** проєкту — нічого не вигадувати. Оркеструє все
команда **`/diploma`** (ланцюг: `thesis-assistant` → `diploma-writer` → `diploma-reviewer`).
Вимоги кафедри — у `vimogi.pdf` (кирилиця витягується погано через шрифт; формат ДСТУ 3008:2015,
TNR 14пт, інтервал 1,5, поля 30/10/20/20, посилання `[N]`, перелік джерел ДСТУ 8302:2015; обсяг не контролювати — вже 80+ с., не скорочувати).
Текст має проходити антиплагіат/анти-ШІ і не містити метакоментарів — **єдиний виняток**:
плейсхолдери рисунків `[РИСУНОК X.X — опис]` / `[ЗАМІНИТИ РИСУНОК X.X — опис]`.
Після кожного сеансу правок у `diploma-changes/` пишеться Markdown-журнал змін (формат —
`diploma-changes/_TEMPLATE.md`): дата, кожна правка «Було → Стало», нові фрагменти, плейсхолдери.
Стан ДО останнього сеансу — у `diploma.backup.docx` (в git не потрапляє).
**Увага:** консоль Windows = cp1251 — кирилицю в stdout не друкувати, працювати через UTF-8 файли (`python-docx`).
